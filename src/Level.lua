...
function Level:init()
    ...
    -- flag that indicates if player has hit anything yet
    self.hit = false
...
function Level:update(dt)
    ...
    -- split two more player aliens when 
    -- user hits space bar and alien has not hit anything
    ...
    if self.launchMarker.launched then
        ...
        local allAlienDoneRolling = true

        local xPos, yPos = self.launchMarker.alien.body:getPosition()
        local xVel, yVel = self.launchMarker.alien.body:getLinearVelocity()
        
        -- if we fired our alien to the left or it's almost done rolling, respawn
        if not(xPos < 0 or (math.abs(xVel) + math.abs(yVel) < 1.5) or 
            (xPos > VIRTUAL_WIDTH*2)) then
            allAlienDoneRolling = false
        end

        -- check splitted table separately
        if self.splitted then
            for m, alien in pairs(self.launchMarker.splitted) do
                xPos, yPos = alien.body:getPosition()
                xVel, yVel = alien.body:getLinearVelocity()

                if xPos < 0 or (math.abs(xVel) + math.abs(yVel) < 1.5)
                    or (xPos > VIRTUAL_WIDTH * 2) then
                else
                    allAlienDoneRolling = false
                end
            end
