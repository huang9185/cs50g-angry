...
function AlienLaunchMarker:split()
    ...
    -- allow splitting once per level
    self.hit = true

    -- add table in alien launch marker to handle spliting 
    self.splitted = {}

    -- spawn the lower alien
    local originX, originY = self.alien.fixture:getBody():getLinearVelocity()
    local lowerAlien = Alien(self.world, 'round', self.alien.body:getX(), self.alien.body:getY(), 'Player')
    lowerAlien.launched = true
    lowerAlien.body:setLinearVelocity(originX, originY + originY*0.2)
    table.insert(self.splitted, lowerAlien)

    -- set the upper alien
    local upperAlien = Alien(self.world, 'round', self.alien.body:getX(), self.alien.body:getY(), 'Player')
    upperAlien.body:setLinearVelocity(originX, originY - originY*0.2)
    table.insert(self.splitted, upperAlien)
end
...
function AlienLaunchMarker:render()
    if not self.launched then
        ...
        -- render splitted aliens too if splitted
        if self.splitted then
            for k, alien in pairs(self.splitted) do
                alien:render()
            end
        end

        -- render the original alien
        self.alien:render()
    end
end